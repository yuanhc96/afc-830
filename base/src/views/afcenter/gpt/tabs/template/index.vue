...
export default {
  ...
  computed: {
    ...VueUtil(this).select([RdcloudTemplateController]).state(),
    loading() {
      return this.RdcloudTemplateController.queryRdcloudTemplatesWithPage.loading
    },
    dialogTitle() {
  ...
  methods: {
    async searchBody() {
      try {
        ...
        const resp = await this.dispatch(
          RdcloudTemplateController.queryRdcloudTemplatesWithPage,
          payload
        )
        return resp
      } catch (err) {
    ...
    async bulkDelete(ids) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        .then(async() => {
          ...
          const resp = await this.dispatch(
            RdcloudTemplateController.deleteByIds,
            payload
          )
          if (!resp.error) {
            this.pmSearch()
    ...
    async createContent(row) {
      ...
      //     RdcloudTemplateController.generateContent,
      //     {
      //         id: row.id,
      //         params: {
      //             selectSystem: '学生管理',
