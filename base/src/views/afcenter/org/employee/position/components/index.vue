<template>
  ...
  >
    <div class="body ml-10 mr-10">
      <el-row :gutter="10">
        <el-col class="h-100" :span="11">
          <div class="block">
            <RoleList :orgData="orgData" :dialogqueryFun="dialogqueryFun" :data.sync="unroleList" v-if="visible" :selectedRows.sync="selections.unroleList" />
          </div>
        </el-col>
        <el-col :span="2">
          <div class="move_btns">
    ...
    <el-row slot="footer" type="flex" justify="center">
      <el-form class="pr-5"><el-button  @click="close">取消</el-button></el-form>
  ...
  methods: {
    ...
    async loadUnroleList() {
      if (this.params) {
        ...
        const resp = await this.dispatch(RoleController.queryRolesWithPage, {
          ...query,
        });
        if (!resp.error) {
