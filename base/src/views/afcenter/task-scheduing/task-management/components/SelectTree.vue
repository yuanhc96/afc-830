<template>
  <el-select
    v-model="finalValue"
    :class="hasValue && clearable ? 'has-selected' : ''"
    v-bind="$attrs"
    :multiple="false"
    :clearable="clearable"
    @clear="clearHandle"
  >
    ...
    <el-option value="default" label="default" class="ide-tree-select__options" :style="`min-width: ${dropdownWidth}px`">
      <el-tree
        ref="ide-tree-select"
        :class="customTreeClass ? customTreeClass : 'ide-tree-select__tree'"
        :show-checkbox="false"
        :accordion="accordion"
        :data="options"
        :props="props"
...
export default {
  ...
  computed: {
    ...
    hasValue() {
      return this.finalValue && ((isString(this.finalValue) && this.finalValue !== '') || (isArray(this.finalValue) && this.finalValue.length))
    },
  },
  watch: {
    value: {
  ...
  methods: {
    ...
    // 初始化值
    initHandle() {
      if (this.valueId && this.$refs['ide-tree-select'] && this.$refs['ide-tree-select'].getNode(this.valueId)) {
        const currentNode = this.$refs['ide-tree-select'].getNode(this.valueId)
        this.valueTitle = currentNode.data[this.showPath ? this.props.pathLabel : this.props.label] // 初始化显示
        this.$emit('update:valuePathLabel', (currentNode.data && currentNode.data.parent) ? currentNode.data.parent.code : null)
      } else {
        this.valueTitle = null
        this.$emit('update:valuePathLabel', null)
      }
    },
    // 切换选项
    handleNodeClick(nodeData, node) {
      // debugger
      if (node.isLeaf) {
        this.$emit('input', nodeData[this.props.value])
    ...
    clearHandle() {
      ...
      this.$emit('update:valuePathLabel', '')
    },
    filterNode(value, data) {
      if (!value) return true
      return data[this.props.label].toLowerCase().indexOf(value.toLowerCase()) !== -1 ||
          (data.parent && data.parent[this.props.label].toLowerCase().indexOf(value.toLowerCase()) !== -1)
    }
  },
}
</script>
