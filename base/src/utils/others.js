...
// 获取时间例如 getDate('y-m-d h:i:s',new Date()); 2020-12-3 15:29:49"
export function getDate(formatStr, fdate) {
  var fTime; var fStr = 'ymdhis'
...
export function substring(str, start, length, suffix = '') {
...
export function getFileSize(size, unit = 'KB') {
  size = size / 1024
  if (size < 1024) {
    size = size.toFixed(2)
    return `${size} ${unit}`
...
export function time19(time = null, length = 19) {
  if (time !== null) return time.substring(0, length)
  else {
    return ''
  }
}
export function timestempToStr(stemp, format = 'YYYY-MM-DD HH:mm') {
  if (stemp) {
...
export function startInterval(id, execute, delay = 3000) {
  stopInterval(id)
  execute.call(this)
  window[id] = window.setInterval(execute, delay)
}

export function stopInterval(id) {
  if (window[id]) {
